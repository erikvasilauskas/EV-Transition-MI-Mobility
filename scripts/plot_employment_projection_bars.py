"""Bar chart comparison of employment projection rates by NAICS."""
...
def melt_rates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    melted["source"] = melted["source"].map(mapping)
...
def build_chart(df_long: pd.DataFrame) -> alt.Chart:
    ...
    chart = (
        ...
        .encode(
            ...
            y=y_enc,
            color=color_enc,
            column=alt.Column("source:N", title="Projection Source"),
            tooltip=[
                "naics_code",
                "naics_title",
                "source",
...
def main() -> None:
    ...
    output_path = repo_root / "reports" / "employment_projection_comparison_bars.html"
    ...
    print(f"Wrote employment projection bar chart to {output_path}")
