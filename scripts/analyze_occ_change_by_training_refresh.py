"""Summarize occupation employment change by education/training group (2024-2030).

This mirrors `analyze_occ_change_by_education_q1.py` but uses the
`ep_edu_training_grouped` classification. Outputs go to the refreshed
2024 annual custom tables.
"""
...
OUTPUT_DIR = Path(
    "data/processed/sam_auto_dashboard_2024_refresh/custom_table_output/occ_change_by_education_2024_2030/training"
)
...
def write_outputs(segment_df: pd.DataFrame, stage_df: pd.DataFrame, total_df: pd.DataFrame) -> None:
    ...
    seg_path = OUTPUT_DIR / "segment_occ_change_by_training_2024_2030.csv"
    stage_path = OUTPUT_DIR / "stage_occ_change_by_training_2024_2030.csv"
    total_path = OUTPUT_DIR / "total_occ_change_by_training_2024_2030.csv"
    segment_df.to_csv(seg_path, index=False)
    stage_df.to_csv(stage_path, index=False)
    total_df.to_csv(total_path, index=False)

    with pd.ExcelWriter(OUTPUT_DIR / "occ_change_by_training_2024_2030.xlsx", engine="xlsxwriter") as writer:
