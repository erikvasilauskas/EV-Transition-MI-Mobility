"""Summarize occupation employment change by education group (2024-2030).

This script reads the segment-level occupation totals produced by
``build_sam_standard_dashboard_data.py`` and aggregates the Michigan
Moody's scenario to report base/target employment, level change, percent
change, and the share of employment accounted for by each of three
education groupings (HS or less, SC or Associate's, BA+). Results are
written to ``data/processed/sam_auto_dashboard/occ_change_by_education``
for segments, stage groupings, and the all-segments total.
"""
...
SEGMENT_FILE = Path("data/processed/sam_auto_dashboard/sam_occ_segment_totals_2024_2034.csv")
SEGMENT_TIMESERIES_FILE = Path("data/processed/sam_auto_dashboard/sam_employment_segment_timeseries.csv")
OUTPUT_DIR = Path("data/processed/sam_auto_dashboard/occ_change_by_education")
...
def compute_change_table(
    ...
    base = (
        ...
        .drop(columns="year")
        .rename(columns={"employment": f"employment_{base_year}"})
    )
    target = (
        df[df["year"] == target_year]
        .drop(columns="year")
        .rename(columns={"employment": f"employment_{target_year}"})
    )
    merged = base.merge(target, on=group_cols + ["education_group"], how="outer")
    base_col = f"employment_{base_year}"
    target_col = f"employment_{target_year}"
    merged[[base_col, target_col]] = merged[[base_col, target_col]].fillna(0.0)
    merged["change_abs"] = merged[target_col] - merged[base_col]
    merged["change_pct"] = np.where(
        merged[base_col] > 0,
        merged["change_abs"] / merged[base_col],
        np.nan,
    )
    base_totals = merged.groupby(group_cols)[base_col].transform("sum")
    target_totals = merged.groupby(group_cols)[target_col].transform("sum")
    merged[f"share_{base_year}"] = np.where(base_totals > 0, merged[base_col] / base_totals, np.nan)
    merged[f"share_{target_year}"] = np.where(target_totals > 0, merged[target_col] / target_totals, np.nan)
    return merged
