...
def build_stage_change_frame(
    ...
    base = base.drop(columns=["year"]).rename(columns={"employment_auto": "employment_auto_2025"})
    ...
    merged["change_level"] = (
        merged["employment_auto_2030"] - merged["employment_auto_2025"]
    )
    merged["change_percent"] = np.where(
        merged["employment_auto_2025"] > 0,
        merged["change_level"] / merged["employment_auto_2025"],
        np.nan,
    )
