...
def load_mcda_shares(path: Path, segment_backcast: Optional[pd.Series] = None) -> pd.DataFrame:
    df = pd.read_csv(path)
    ...
    df["ep_openings_annual_avg"] = pd.to_numeric(df["ep_openings_annual_avg"], errors="coerce")
    df["empl_2021"] = pd.to_numeric(df.get("empl_2021"), errors="coerce")

    if segment_backcast is not None:
        seg_lookup = segment_backcast.astype(float)
        df["segment_backcast_total"] = df["segment_id"].map(seg_lookup)
        df["empl_2021_backcast"] = df["segment_backcast_total"] * df["share_2024"]
        df["empl_2021"] = np.where(
            df["empl_2021_backcast"].notna(),
            df["empl_2021_backcast"],
            df["empl_2021"],
        )
    ...
    df["empl_2021"] = pd.to_numeric(df["empl_2021"], errors="coerce")
    return df
...
def interpolate_shares(base: pd.DataFrame, target: Optional[pd.DataFrame], years: list[int]) -> pd.DataFrame:
    ...
    shares["ep_openings_annual_avg"] = pd.to_numeric(shares["ep_openings_annual_avg"], errors="coerce")
    if "empl_2021" in shares.columns:
        shares["empl_2021"] = pd.to_numeric(shares["empl_2021"], errors="coerce")
    else:
        shares["empl_2021"] = np.nan
    ...
    for _, row in share_df.iterrows():
        ...
        for method in methodologies:
            ...
            records.append({
                ...
                "empl_2021": row.get("empl_2021", np.nan),
...
def main() -> None:
    ...
    parser.add_argument("--segment-historical-path", type=Path, default=Path("data/processed/mi_qcew_segment_employment_timeseries_coreauto_extended_compare.csv"))
    parser.add_argument("--backcast-year", type=int, default=2021)
    ...
    base_year = years[0]
    target_year = years[-1]
    segment_backcast = None
    try:
        segment_history = pd.read_csv(args.segment_historical_path)
    except FileNotFoundError:
        segment_history = None
    if segment_history is not None:
        required_cols = {"segment_id", "year", "employment_qcew"}
        if required_cols.issubset(segment_history.columns):
            hist = segment_history.copy()
            hist["segment_id"] = pd.to_numeric(hist["segment_id"], errors="coerce").astype("Int64")
            hist["year"] = pd.to_numeric(hist["year"], errors="coerce").astype("Int64")
            hist["employment_qcew"] = pd.to_numeric(hist["employment_qcew"], errors="coerce")
            hist = hist.dropna(subset=["segment_id", "year", "employment_qcew"])
            hist = hist[hist["year"] == args.backcast_year]
            if "value_type" in hist.columns:
                hist["value_type"] = hist["value_type"].astype(str).str.lower()
                hist = hist[hist["value_type"] == "qcew"]
            if not hist.empty:
                segment_backcast = hist.groupby("segment_id")["employment_qcew"].sum(min_count=1)

    mcda = load_mcda_shares(args.mcda_path, segment_backcast)
