...
BASE_YEAR = 2025
BASE_PERIOD = pd.Timestamp(BASE_YEAR, 3, 1)
...
def load_monthly_base_lookup(path: Path) -> dict[str, float]:
    if not path.exists():
    ...
    if df.empty:
        raise ValueError(f"No NAICS detail rows available for {BASE_PERIOD.date()}")
...
def load_base_dataframe(repo_root: Path) -> pd.DataFrame:
    ...
    base_lookup = load_monthly_base_lookup(repo_root / MONTHLY_QCEW_PATH)
    if base_lookup:
...
def main() -> None:
    ...
    output_dir = repo_root / "data" / "processed" / "sam_auto_dashboard_2025_Q1"
    ...
    segment_change_path = output_dir / "segment_change_2025_2030.csv"
    stage_change_path = output_dir / "stage_change_2025_2030.csv"
    ...
    occ_full_path = output_dir / "sam_occ_segment_totals_2025_2034.csv"
